...
def date_to_month(d):
    ...
    d['data'] = pd.to_datetime(d['date'])
    d['year'] = d['date'].apply(lambda x: x.year)
    d['month'] = d['date'].apply(lambda x: x.month)
    d['day'] = d['date'].apply(lambda x: x.day)
    d['counts'] = 1
    d['month'] = d['year'].map(str) + "-" + d['month'].map(str)
    
    """
    reference of .map(str) method to transform data type int to string:
...
def pivot_months_pandas(data):
    ...
    date_to_month(data_copy)     #modify the date to month we going to use in following analysis
